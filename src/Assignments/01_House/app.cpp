...
void SimpleShapeApplication::init() {
    ...
    OGL_CALL(glBufferData(GL_ARRAY_BUFFER, std::size(vertices) * sizeof(GLfloat), vertices.data(), GL_STATIC_DRAW));
    ...
    OGL_CALL(glClearColor(0.5f, 0.5f, 0.5f, 1.0f));
...
void SimpleShapeApplication::frame() {
    ...
    OGL_CALL(glBindVertexArray(vao_));
    OGL_CALL(glDrawArrays(GL_TRIANGLES, 0, 12));
    OGL_CALL(glBindVertexArray(0));
}
